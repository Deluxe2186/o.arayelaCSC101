...
fn main() {
    ...
    let mut v2 = vec![exp[hghindex].clone()];

    for z in 0..exp.len()
    {
        let index = z as usize;
        
        if exp[index] == hghyear && hghindex != index
        {
            morethan = true;
            v.push(names[index].clone());
            v2.push(exp[index].clone());
        }
    }
    ...
    for i in 0..names.len()
    {
        ...
        println!("{}.Applicant Name:   {}         Years of programming experience:   {}", i+1, names[index],exp[index]);
    }
